...
9월 원티드 프리온보딩 챌린지 (typescript)의 사전과제 수행을 위해 JSDoc을 설치 후 사용해봤다.

1. JSDoc이란?

- 자바스크립트 코드에 대한 주석, 타입 지정 등의 설명을 달 수 있는 마크업 언어
- 그런 동시에, js파일을 문서화해서 시각적으로 볼 수 있게 만들어주는 모듈의 이름이기도 하다.
- `/** */` 사이에 내용을 입력해 작성할 수 있다. VSCode 등의 에디터에서도 자동완성을 지원함.
- 타입스크립트에서 사용할 수 있는 타입 지정, 타입 추론등의 기능 중 상당수를 동일하게 사용할 수 있는 것으로 보임
- 필수 요구사항에 맞추어 Todo List 서비스의 기본 구조를 설계해봤는데, javascript로 api를 제대로 만들어 본 적이 없는 나로서는 온통 혼돈 그 자체.. javascript의 class와 constructor도 처음 써봤다.
- 다음과 같은 형태로 커스텀 타입 지정이 가능하다. 커스텀 타입을 이용해 데이터 모델의 기본 구조를 만들었다. `[]`를 이용하면 해당 타입이 optional 값이라는 의미가 된다.
...
- 기본이 될 Todo Item 객체를 사용하기 위한 클래스를 만들었다.
- 이 안에 필요한 api 메서드를 바인딩하는 식으로 만들고 싶은데, 아직 구체적인 선언 방식을 잘 모르겠다. 일단 Docs 뚫어져라 뜯어보고 이것저것 시도해보는 중

```js
import { TodoInputsType } from "./typedef";
 ...
 */
class Todo {
  /** @param {TodoInputsType} TodoInputs */
  constructor(TodoInputs) {
    this.TodoItem = TodoInputs;
  }
}
```
...
- JSDoc의 예시 코드를 보고 prototype을 이용해 필요한 api 목록을 index.js에 만들었다. 그런데 CRUD 에 해당하는 메소드가 각각의 분류별로 정리되지 않고 모조리 흩뿌려져 있어서 보기 안좋다는 생각이 든다.
- class 안에 CRUD에 해당하는 메소드를 분류해 넣고, 해당 api를 실행하는 함수도 선언해보자.
...
- 컴퓨터 시스템의 **자원을 효율적으로 관리한다.** (중요‼️‼️‼️)
